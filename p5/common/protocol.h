...
/// LEN_PASS.
///
/// @rblock   padR(enc(pubkey, "KVT".aeskey.length(@ablock)))
/// @ablock   enc(aeskey, @u."\n".@p)
/// @response enc(aeskey, "OK".length(@l).@l).<EOF> -- Success
///           enc(aeskey, error_code).<EOF>         -- Error (see @errors)
///           ERR_CRYPTO.<EOF>                      -- Error (see @errors)
...
////////////////////////////////////////////////////////////////
// Below are the additions for Assignment #5
////////////////////////////////////////////////////////////////

/// Maximum length of a function name
const int LEN_FNAME = 32;

/// Maximum length of a shared object (16MB)
const int LEN_SO = 16777216;

/// Allow user @u (with password @p) to invoke a map() and reduce() function
/// pair over the key/value store.
///
/// The user name (@u) and user password (@p) must conform to LEN_UNAME and
/// LEN_PASS.  The map/reduce (@m) name must be no more than LEN_FNAME
/// characters.
///
/// @rblock   padR(enc(pubkey, "KMR".aeskey.length(@ablock)))
/// @ablock   enc(aeskey, @u."\n".@p."\n".@m)
/// @response enc(aeskey, "OK".length(@l).@l).<EOF> -- Success
///           enc(aeskey, error_code).<EOF>         -- Error (see @errors)
///           ERR_CRYPTO.<EOF>                      -- Error (see @errors)
...
///           ERR_MSG_FMT     -- Server unable to extract @u or @p or @m
///           ERR_FUNC        -- @m could not be found
///           ERR_CRYPTO      -- Server could not decrypt @ablock
///           ERR_QUOTA_REQ   -- Client exceeded request quota
///           ERR_QUOTA_DOWN  -- Client exceeded download bandwidth quota
///           ERR_SERVER      -- Internal Server Error
const std::string REQ_KMR = "KMR";

/// Allow user @u (with password @p) to register a .so (@s) containing map() and
/// reduce() functions, using the name @m.
///
/// The user name (@u) and user password (@p) must conform to LEN_UNAME and
/// LEN_PASS.  @m must be no more than LEN_FNAME characters.  The user (@u) must
/// be the systen administrator.  The .so file (@s) must be no more than LEN_SO
/// bytes.
///
/// @rblock   padR(enc(pubkey, "KVF".aeskey.length(@ablock)))
/// @ablock   enc(aeskey, @u."\n".@p."\n".@m."\n".length(@s).@s)
/// @response enc(aeskey, "OK").<EOF>       -- Success
///           enc(aeskey, error_code).<EOF> -- Error (see @errors)
///           ERR_CRYPTO.<EOF>              -- Error (see @errors)
/// @errors   ERR_LOGIN       -- @u is not a valid user
///           ERR_LOGIN       -- @p is not @u's password
///           ERR_LOGIN       -- @u is not an administrator
///           ERR_KEY         -- @k already has a value
///           ERR_MSG_FMT     -- Server unable to extract @u or @p or @m or @s
...
///           ERR_QUOTA_UP    -- Client exceeded upload bandwidth quota
///           ERR_SERVER      -- Internal Server Error
const std::string REQ_KVF = "KVF";

/// Response code to indicate that a function could not be found for KMR, or
/// already existed for KVF
const std::string RES_ERR_FUNC = "ERR_FUNC";

/// Response code to indicate that a .so doesn't have map() and reduce()
/// functions defined in it
const std::string RES_ERR_SO = "ERR_SO";

/// Response code to indicate that the server had an internal error, such as a
/// bad read from a file or failure to fork()
